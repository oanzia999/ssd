...
2. Confidentiality: Fernet Encryption (AES) for PII.
3. Integrity: Immutable Audit Logs.
4. Availability: Rate Limiting.
5. Input Validation: Bleach sanitization.
...
def register():
    ...
    try:
        ...
        conn.close(); flash('Email registered.'); return redirect(url_for('register_page'))
    conn.close()
...
def dashboard():
    ...
    doctors = [{"id": str(d['_id']), "fullname": decrypt_data(d['fullname'])} for d in db.patients.find({"role": "doctor"})]
    ...
    return render_template('dashboard.html', user=user, risk_score=risk, advice_list=advice, appointments=appointments, doctors=doctors)
...
def doctor_dashboard():
    ...
    return render_template('doctor_dashboard.html', patients=patients, pending_appointments=pending, confirmed_appointments=confirmed, doctor_name=session['fullname'])
...
def download_report():
    ...
    # Fills space between columns and footer
    y_insights = 175
    pdf.set_fill_color(245, 245, 245)
    ...
    pdf.set_text_color(50, 50, 50)
    # Multi-cell auto-wraps the long generated text
...
def admin_dashboard():
    ...
    db = get_mongo_db()
    p_count = db.patients.count_documents({"role": "patient"})
    d_count = db.patients.count_documents({"role": "doctor"})

    doctors = []
    for d in db.patients.find({"role": "doctor"}):
        d['fullname'] = decrypt_data(d.get('fullname'))
        doctors.append(d)
    ...
    return render_template('admin_dashboard.html',
                           patient_count=p_count, doctor_count=d_count,
                           doctors=doctors, patients=patients, logs=logs)

@app.route('/admin/add_doctor', methods=['POST'])
def add_doctor():
    if session.get('role') != 'admin': return redirect(url_for('login_page'))
    name = clean_input(request.form.get('fullname'))
    ...
    conn = get_sqlite_conn()
    try:
        conn.execute('INSERT INTO users (email, password_hash, role) VALUES (?, ?, ?)', (email, hashed, 'doctor'))
        conn.commit()
    except:
        conn.close(); flash('Email taken'); return redirect(url_for('admin_dashboard'))
    conn.close()

    get_mongo_db().patients.insert_one({"email": email, "fullname": encrypt_data(name), "role": "doctor"})
    flash('Doctor Added.')
...
def delete_user(user_id):
    if session.get('role') != 'admin': return redirect(url_for('login_page'))
    db = get_mongo_db()
    u = db.patients.find_one({"_id": ObjectId(user_id)})
    if u:
        conn = get_sqlite_conn()
